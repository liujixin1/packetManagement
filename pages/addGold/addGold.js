...
Page({
  ...
  uploadImg() {
    ...
    wx.chooseImage({
      ...
      success: function (res) {
        ...
        //拼接成图片名
        let keepname =  'goldImg/'+time + '.' + fileExt;
        wx.cloud.uploadFile({
          cloudPath: keepname,
  ...
  onLoad: function (options) {
    ...
    // })
    if (options.id) {
      db.collection('gold').doc(options.id).get().then(res => {
        console.log(res)
        wx.hideLoading()
        that.setData({
          form: res.data,
